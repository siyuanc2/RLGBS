...
from .hf_doc import add_start_docstrings
...
class RLCBSMaxNTokensLogitsProcessor(LogitsProcessor):
    r"""
    [`LogitsProcessor`] that enforces a maximum number of tokens of certain kind to be generated 
    in a sequence. Used in RLCBS (Reinforcement Learning Constrained Beam Search).

    Args:
        max_n_tokens (`int`):
            The maximum number of tokens of certain kind to be generated in a sequence.
        token_ids (`List[int]`):
            The ids of the tokens of certain kind.
    """
    def __init__(self, max_n_tokens: int, token_ids: List[int]):
        self.max_n_tokens = max_n_tokens
        self.test_elements = torch.tensor(token_ids, requires_grad=False)

    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        # if self.test_elements is on a different device than input_ids, move it to the same device
        if self.test_elements.device != input_ids.device:
            self.test_elements = self.test_elements.to(input_ids.device)
        # count the number of tokens of certain kind in the sequence
        n_tokens = torch.sum(torch.isin(input_ids, self.test_elements), dim=1)
        # find the indices of the sequences that exceed the maximum number of tokens
        filtered_indices = torch.where(n_tokens >= self.max_n_tokens)[0]
        # if filtered_indices is not empty, set the scores of the sequences to -inf
        if filtered_indices.numel():
            ii, jj = torch.meshgrid(filtered_indices, self.test_elements, indexing='ij')
            scores[ii, jj] = -float("inf")
        return scores

class RLCBSDiscardFailedSequencesLogitsProcessor(LogitsProcessor):
    r"""
    [`LogitsProcessor`] that suppresses the scores of sequences that is known to have failed.
    Used in RLCBS (Reinforcement Learning Constrained Beam Search), where failed sequences are
    detected by the presence of a special token.

    Args:
        failed_token_id (`int`):
            The id of the failed token.
    """
    def __init__(self, failed_token_id: int) -> None:
        if failed_token_id >= 0:
            raise ValueError(f"`failed_token_id` has to be a negative integer, but is {failed_token_id}")
        self.failed_token_id = failed_token_id

    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        # mask the scores if the sequence is known to have failed
        mask = torch.any(input_ids == self.failed_token_id, dim=1)
        scores = torch.where(mask.unsqueeze(-1), -float("inf"), scores)
        return scores

class RLCBSAggregateProbabilityLogitsProcessor(LogitsProcessor):
    r"""
    [`LogitsProcessor`] that sets a action's probability to the sum of all actions of the same module type.
    Used in RLCBS (Reinforcement Learning Constrained Beam Search).
	Expected to be used immediately before a RLCBSTemperatureContinuityLogitsProcessor, where probabilities 
	of all but one temperature will be set to -inf.

    Args:
        n_temp (`int`):
            Number of discrete temperature levels used in the smart dryer optimization problem.
        n_module (`int`):
            Number of module types used in the smart dryer optimization problem.
        included_modules (`List[int]`):
            The module types for which this logits processor is effective.

    """
    def __init__(self, n_temp: int, n_module: int, included_modules: List[int]) -> None:
        if n_temp < 1:
            raise ValueError(f"`n_temp` has to be a positive integer, but is {n_temp}")
        if n_module < 1:
            raise ValueError(f"`n_module` has to be a positive integer, but is {n_module}")
        if not all(isinstance(x, int) and x < n_module for x in included_modules):
            raise ValueError(f"`included_modules` has to be a list of integers less than `n_module`, but is {included_modules}")
        self.n_temp = n_temp
        self.n_module = n_module
        self.included_modules = included_modules if included_modules else []

    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        # If the input_ids is empty or does not contain any positive integers, then do nothing
        if not torch.any(input_ids > 0):
            return scores
        # If no module type is included, then do nothing
        if not self.included_modules:
            return scores
        for i in self.included_modules:
            # # Find the indices of all actions of the same module type
            # same_module_indices = torch.where(input_ids[:, -1] // self.n_temp == i)[0]
            # Sum the probabilities of all actions of the same module type
            scores[:, i*self.n_temp:(i+1)*self.n_temp] = torch.log(torch.exp(scores[:, i*self.n_temp:(i+1)*self.n_temp]).sum(dim=-1).unsqueeze(-1))
        return scores

class RLCBSTemperatureContinuityLogitsProcessor(LogitsProcessor):
    r"""
    [`LogitsProcessor`] that enforces a continuity constraint on the temperature of the generated sequences.
    Used in RLCBS (Reinforcement Learning Constrained Beam Search).
    In the smart dryer optimization problem, the vocabulary is of size (n_temp * n_module)
    where T = action % n_temp and M = action // n_temp.
    This processor ensures that the temperature of the generated sequences is within a certain tolerance of the
    previous module's temperature setting, i.e. |T - T_prev| <= tolerance.
    ...
    Args:
        ...
        n_module (`int`):
            Number of module types used in the smart dryer optimization problem.
        tolerance (`int`):
            The tolerance of the temperature constraint.
        included_modules (`List[int]`):
            The module types that are excluded from the temperature constraint.

    """
    def __init__(self, n_temp: int, n_module: int, tolerance: int, included_modules: Optional[List[int]] = None) -> None:
        if tolerance < 0:
            raise ValueError(f"`tolerance` has to be a non-negative integer, but is {tolerance}")
        if n_temp < 1:
            raise ValueError(f"`n_temp` has to be a positive integer, but is {n_temp}")
        if n_module < 1:
            raise ValueError(f"`n_module` has to be a positive integer, but is {n_module}")
        ...
        # Store all included module types
        if not included_modules:
            included_modules = []
        self.included_module_type = included_modules
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if not torch.any(input_ids > 0):
            return scores
        # If no module type is included, then do nothing
        if not self.included_module_type:
            return scores
        # Extract the temperature and module type from the input_ids for all beams
        temperature = input_ids[:, -1] % self.n_temp
        # module_type = input_ids[:, -1] // self.n_temp
        batch_size = input_ids.shape[0]
        # Initialize the array of all possible temperatures
        all_temperatures = torch.arange(self.n_temp*self.n_module, device=scores.device, requires_grad=False).view(self.n_module, -1).expand(batch_size, self.n_module, self.n_temp)
        all_mask = all_temperatures.new_zeros(all_temperatures.shape, dtype=torch.bool)
        # Loop through all included module types and mask the scores if the temperature constraint is violated
        for i in self.included_module_type:
            # Find the indices of next tokens that will violate the temperature constraint
            all_mask[:, i, :] = torch.abs(all_temperatures[:, i, :] % self.n_temp - temperature.unsqueeze(-1)) > self.tolerance
        # Set elements in `scores` to -inf according to the computed mask
        scores = torch.where(all_mask.view(-1, self.n_module*self.n_temp), -float("inf"), scores)
        return scores
