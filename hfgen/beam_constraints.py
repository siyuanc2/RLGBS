...
class SequentialDisjunctiveConstraint(Constraint):
    r"""
    [Constraint] enforcing that an ordered sequence of tokens is included in the output.
    The tokens must be generated in the order they are provided, but do not need to be contiguous.
    This constraint allows for choices between multiple tokens at each step.
    e.g. for input [[1, 2], [3]], the output can be [1, 3] or [2, 3].

    Args:
        token ids (`List[List[int]]`):
            List of ids of tokens that must be generated sequentially.
    """
    ...
    def __init__(self, nested_token_ids: List[List[int]]):
        ...
        self.seqlen = len(nested_token_ids)
        self.fulfilled_idx = -1  # the index of the currently fulfilled step
        self.completed = False
    ...
    def does_advance(self, token_id: int):
        ...
        return token_id in self.token_ids[self.fulfilled_idx + 1]
    ...
    def reset(self):
        self.completed = False
        # because the tokens do not need to be contiguous, no need to reset the fulfilled_idx

    def remaining(self):
        return self.seqlen - (self.fulfilled_idx + 1)

    def copy(self, stateful=False):
        new_constraint = SequentialDisjunctiveConstraint(self.token_ids)
...
class OrderedMultiPhrasalConstraint(Constraint):
    r"""
    [Constraint] enforcing that an ordered sequence of phrases is included in the output. 
	The order of phrases is not mutable which separates this from a disjunctive lexical constraint.

	Args:
		token ids (`List[List[int]]`):
			List of List of ids of tokens that must be generated sequentially.
    """

    def __init__(self, token_ids: List[List[int]]):
        super(Constraint, self).__init__()

        if not (isinstance(token_ids, list) and all(isinstance(phrase, list) for phrase in token_ids)) or len(token_ids) == 0:
            raise ValueError(f"`token_ids` has to be a non-empty List[List[int]], but is {token_ids}.")
        if any(any((not isinstance(token_id, int) or token_id < 0) for token_id in phrase) for phrase in token_ids):
            raise ValueError(f"Each list in `token_ids` has to be a list of positive integers, but is {token_ids}.")

        self.unprocessed_token_ids = token_ids
        self.token_ids = []
        phrase_end_idx = []
        current_idx = 0
        for phrase in token_ids:
            if phrase:
                self.token_ids.extend(phrase)
                current_idx += len(phrase)
                phrase_end_idx.append(current_idx - 1)
            else:
                raise ValueError(f"Each phrase in `token_ids` has to be a non-empty list, but is {token_ids}.")

        self.phrase_end_idx = phrase_end_idx
        self.seqlen = len(self.token_ids)
        self.fulfilled_idx = -1  # the index of the currently fulfilled step
        self.fulfilled_phrase_idx = -1 # the index of the currently fulfilled phrase
        self.completed = False
    ...
    def update(self, token_id: int):
        ...
        if self.does_advance(token_id):
            self.fulfilled_idx += 1
            stepped = True
            if self.fulfilled_idx in self.phrase_end_idx:
                self.fulfilled_phrase_idx += 1
            if self.fulfilled_idx == (self.seqlen - 1):
                completed = True
            self.completed = completed
        else:
    ...
    def reset(self):
        self.completed = False
        if self.fulfilled_phrase_idx == -1:
            self.fulfilled_idx = -1
        else:
            self.fulfilled_idx = self.phrase_end_idx[self.fulfilled_phrase_idx]

    def remaining(self):
        return self.seqlen - (self.fulfilled_idx + 1)

    def copy(self, stateful=False):
        new_constraint = OrderedMultiPhrasalConstraint(self.unprocessed_token_ids)

        if stateful:
            new_constraint.fulfilled_idx = self.fulfilled_idx
            new_constraint.fulfilled_phrase_idx = self.fulfilled_phrase_idx
            new_constraint.completed = self.completed
